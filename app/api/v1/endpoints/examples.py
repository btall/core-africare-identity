...
from app.models.example import Example, ExampleCreate, ExampleResponse, ExampleUpdate
...
async def create_example(
    example_data: ExampleCreate, db: AsyncSession = Depends(get_session)
) -> ExampleResponse:
...
async def get_example(example_id: int, db: AsyncSession = Depends(get_session)) -> ExampleResponse:
...
async def update_example(
    example_id: int, example_data: ExampleUpdate, db: AsyncSession = Depends(get_session)
) -> ExampleResponse:
...
async def delete_example(example_id: int, db: AsyncSession = Depends(get_session)):
    ...
    # Publier l'événement
    await publish(
        "identity.example.deleted",
        {"example_id": example_id, "timestamp": datetime.now(UTC).isoformat()},
    )
